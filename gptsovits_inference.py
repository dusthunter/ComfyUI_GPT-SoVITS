...
cnhubert_base_path = os.path.join(
    parent_directory, "GPT_SoVITS/pretrained_models/chinese-hubert-base"
)
bert_path = os.path.join(
    parent_directory, "GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large"
)

path_sovits_v3 = os.path.join(
    parent_directory, "GPT_SoVITS/pretrained_models/s2Gv3.pth"
)
path_sovits_v4 = os.path.join(
    parent_directory, "GPT_SoVITS/pretrained_models/gsv-v4-pretrained/s2Gv4.pth"
)
...
def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    ...
    if if_lora_v3 == True and is_exist == False:
        info = i18n("SoVITS %s 底模缺失，无法加载相应 LoRA 权重" % model_version)
        print(info)
...
def get_tts_wav(
    ...
    with torch.no_grad():
        ...
        if ref_sample_rate != 16000:
            wav16k = torchaudio.transforms.Resample(
                orig_freq=ref_sample_rate, new_freq=16000
            )(wav16k)
    ...
    with torch.no_grad():
        ...
        fea_ref_pre, ge_pre = vq_model.decode_encp(
        ...
        if ref_sample_rate != tgt_sr:
            ref_wav = resample(ref_wav, ref_sample_rate, tgt_sr)
        mel2_pre = mel_fn(ref_wav) if model_version == "v3" else mel_fn_v4(ref_wav)
    ...
    for i_text, text in enumerate(texts):
        ...
        fea_todo, ge = vq_model.decode_encp(
            pred_semantic, phoneme_ids1, refer, ge_pre, speed
        )  # Using pre-computed ge_pre
